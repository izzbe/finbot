...
async def post_historical(request: BarDataRequest, conn=Depends(get_conn)):
    payload: list[Bar] = app.state.fetcher.get_data(
        request.symbol, request.start, request.end, request.timespan
    )
...
async def get_historical(
    ...
    if timespan not in {"min", "hr", "day", "week", "month", "year"}:
        raise HTTPException(
            422, "timespan must be one of min, hr, day, week, month, year"
        )
